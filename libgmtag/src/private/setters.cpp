...
static inline char *set_generic (std::string &target, char *buffer) {
    buffer = skip_spaces(buffer);
    char *tmp = static_cast<char *>(malloc(MAX_FIELD_LENGTH));
    buffer = into_buffer_until_newline(
        buffer,
        tmp,
        MAX_FIELD_LENGTH
    );
...
char *set_album (GmTagDef &tag, char *buffer) {
    return set_generic(tag.album, buffer);
}

char *set_title (GmTagDef &tag, char *buffer) {
    return set_generic(tag.title, buffer);
}

char *set_composer (GmTagDef &tag, char *buffer) {
    return set_generic(tag.composer, buffer);
}

char *set_company (GmTagDef &tag, char *buffer) {
    return set_generic(tag.company, buffer);
}

char *set_publisher (GmTagDef &tag, char *buffer) {
    return set_generic(tag.publisher, buffer);
}

char *set_artist (GmTagDef &tag, char *buffer) {
    return set_generic(tag.artist, buffer);
}

char *set_sequencer (GmTagDef &tag, char *buffer) {
    return set_generic(tag.sequencer, buffer);
}

char *set_engineer (GmTagDef &tag, char *buffer) {
    return set_generic(tag.engineer, buffer);
}

char *set_date (GmTagDef &tag, char *buffer) {
    buffer = skip_spaces(buffer);
...
char *set_ripper (GmTagDef &tag, char *buffer) {
    return set_generic(tag.ripper, buffer);
}

char *set_tagger (GmTagDef &tag, char *buffer) {
    return set_generic(tag.tagger, buffer);
}

char *str_to_time (char *buffer, GmTagTimeDef &time) {
...
char *set_length (GmTagDef &tag, char *buffer) {
    buffer = skip_spaces(buffer);
...
char *set_fade (GmTagDef &tag, char *buffer) {
    buffer = skip_spaces(buffer);
    GmTagTimeDef time = GmTagTimeDef{};
    buffer = str_to_time(buffer, time);
    tag.fade = time;
...
char *set_comment (GmTagDef &tag, char *buffer) {
    buffer = skip_spaces(buffer);
    char *tmp = static_cast<char *>(malloc(MAX_FIELD_LENGTH));
    buffer = into_buffer_until_newline(
        buffer,
...
char *set_copyright (GmTagDef &tag, char *buffer) {
    return set_generic(tag.copyright, buffer);
}

char *set_track (GmTagDef &tag, char *buffer) {
    buffer = skip_spaces(buffer);
    std::string tmp;
    char cur_char = *buffer++;
    while (is_number(cur_char)) {
